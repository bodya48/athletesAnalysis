...
- (void)requestForChain17BasedOnFileID:(NSString *)fileID
                           accessToken:(id)tokenProvider
                        withCompletion:(void (^)(NSString *vitaminDValue))completion {
    
    NSLog(@"starting request for chains17: power marker");
    [tokenProvider token:^(SQToken *token, NSString *accessToken) {
        ...
        // v2 protocol
        [appChains getReportWithApplicationMethodName:@"Chain17"
                                     withDatasourceId:fileID
                                     withSuccessBlock:^(Report *result) {
                                         
                                         completion([self parseReportForChain17:result]);
                                     }
                                     withFailureBlock:^(NSError *error) {
                                         if (error) {
                                             NSLog(@"[appChain17 Error] %@", error);
                                             completion(nil);
                                         } else {
                                             completion(nil);
                                         }
...
- (void)requestForChain18BasedOnFileID:(NSString *)fileID
                           accessToken:(id)tokenProvider
                        withCompletion:(void (^)(NSString *vitaminDValue))completion {
    
    NSLog(@"starting request for chains18: endurance marker");
    [tokenProvider token:^(SQToken *token, NSString *accessToken) {
        if (!accessToken || [accessToken length] == 0) {
            completion(nil);
            return;
        ...
        // v2 protocol
        [appChains getReportWithApplicationMethodName:@"Chain18"
                                     withDatasourceId:fileID
                                     withSuccessBlock:^(Report *result) {
                                         
                                         completion([self parseReportForChain18:result]);
                                     }
                                     withFailureBlock:^(NSError *error) {
                                         if (error) {
                                             NSLog(@"[appChain18 Error] %@", error);
                                             completion(nil);
                                         } else {
                                             completion(nil);
                                         }
...
- (NSString *)parseReportForChain17:(Report *)result {
    NSString *value;
...
- (NSString *)parseReportForChain18:(Report *)result {
    NSString *value;
